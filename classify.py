...
import sys # for system and os related tasks
import time # for fps measurment 
...
def run(model: str, max_results: int, score_threshold: float, num_threads: int,
        message_threshold:int,enable_edgetpu: bool, camera_id: int, width: int, height: int) -> None:
  ...
  Args:
      ...
      message_threshold: The score threshold at which message to device should be sent.
  ...
  # Pushbullet object to send messages
  try:
    pb = Pushbullet('o.6VQ9bu8yUtfRkP52qPWZE7EQUBeJDur6')
  except:
      print("Could not connect to local pushbullet repository")
  device = pb.get_device('Samsung SM-M215G')
